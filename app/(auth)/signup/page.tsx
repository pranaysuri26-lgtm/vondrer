...
export default function SignupPage() {
  ...
  function canAdvance(): boolean {
    switch (step) {
      ...
      case 'past_trips': return true // always optional
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
              <div className="text-center mb-6">
                ...
                <div className="text-white font-semibold text-lg">
                  {OFFBEAT_LABELS[form.offbeat_score].label}
                </div>
                <div className="text-white/45 text-sm mt-1">
                  {OFFBEAT_LABELS[form.offbeat_score].sub}
                </div>
              </div>
            ...
            {form.past_trips.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {form.past_trips.map(name => (
                  <span
                    key={name}
                    className="flex items-center gap-1.5 bg-[#C97552]/15 border border-[#C97552]/30 text-white/80 text-sm px-3 py-1.5 rounded-full"
                  >
                    {name}
                    <button
                      type="button"
                      onClick={() => removePastTrip(name)}
