...
function sessionLabel(s: string) {
  switch (s) {
    ...
    case 'night':          return { label: 'Night',            color: 'text-[#5C564E] bg-white border-[#E8E0D6]',             icon: '🌙' }
    default:               return { label: s,                  color: 'text-[#5C564E] bg-white border-[#E8E0D6]',             icon: '📷' }
  }
}
...
function SunTimingCard({ sun }: { sun: PhotoSunTimes }) {
  ...
  return (
    <div className="rounded-2xl border border-amber-400/20 bg-amber-400/5 overflow-hidden mb-5">
      <div className="px-4 pt-4 pb-3 border-b border-[#EDE5D8]">
      ...
      <div className="grid grid-cols-2 divide-x divide-white/6">
        ...
        <div className="px-4 py-3 space-y-2">
          <p className="text-[10px] text-[#8A7E6E] font-label tracking-widest uppercase">Morning</p>
          {sessions.filter(s => s.period === 'AM').map((s, i) => (
            <div key={i} className={`border-l-2 pl-2.5 ${s.color}`}>
              <p className="text-[#3A3430] text-xs font-medium">{s.emoji} {s.label} hour</p>
        ...
        {/* Evening */}
        <div className="px-4 py-3 space-y-2">
          <p className="text-[10px] text-[#8A7E6E] font-label tracking-widest uppercase">Evening</p>
    ...
    <button
      ...
      className={`w-full text-left rounded-2xl border transition-all duration-200 overflow-hidden ${
        selected
          ...
          : 'border-[#E8E0D6] bg-white hover:border-[#C97552]/30 hover:bg-[#F5F0EA]'
      }`}
    >
      <div className="px-4 py-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-[#1A1A1A] text-sm font-semibold leading-snug">{spot.name}</p>
              </div>
        ...
        {selected && (
          <div className="mt-4 space-y-4">
            {/* Composition */}
            <div>
              <p className="text-[10px] text-[#8A7E6E] font-label tracking-widest uppercase mb-1.5">Composition</p>
              <p className="text-[#2A2420] text-sm leading-relaxed">{spot.composition}</p>
            </div>

            {/* Locals tip */}
            <div className="rounded-xl bg-white border border-[#E8E0D6] px-3.5 py-3">
              <p className="text-[10px] text-[#8A7E6E] font-label tracking-widest uppercase mb-1">Where locals stand</p>
              <p className="text-[#3A3430] text-sm leading-relaxed">{spot.locals_tip}</p>
            </div>

            {/* Light + lens row */}
            <div className="grid grid-cols-2 gap-3">
              <div>
                <p className="text-[10px] text-[#8A7E6E] font-label tracking-widest uppercase mb-1">The light</p>
                <p className="text-[#5C564E] text-xs leading-relaxed">{spot.light_note}</p>
              </div>
              <div>
                <p className="text-[10px] text-[#8A7E6E] font-label tracking-widest uppercase mb-1">Lens</p>
...
function StopCard({ stop, index }: { stop: TripStop; index: number }) {
  ...
  return (
    <div className="rounded-2xl border border-[#E8E0D6] bg-white overflow-hidden">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-[#1A1A1A] text-sm font-semibold leading-snug">{stop.name}</p>
                <p className="text-[#5C564E] text-xs mt-0.5">{stop.city}, {stop.state}</p>
              </div>
      ...
      {open && (
        <div className="px-5 pb-4 border-t border-[#EDE5D8] pt-3 space-y-3">
          <p className="text-[#4A4440] text-sm leading-relaxed">{stop.why}</p>
          <div className="flex flex-wrap gap-2">
            {stop.dietary_fit && dietaryBadge(stop.dietary_fit)}
            {stop.distance_note && <span className="text-[11px] font-label px-2.5 py-1 rounded-full border border-[#E8E0D6] bg-white text-[#5C564E]">📍 {stop.distance_note}</span>}
            {stop.open_note && <span className="text-[11px] font-label px-2.5 py-1 rounded-full border border-[#E8E0D6] bg-white text-[#5C564E]">🕐 {stop.open_note}</span>}
          </div>
          {stop.lat && stop.lng && (
...
function PlaceCard({ place, index, selected, onSelect }: { place: LocalPlace; index: number; selected: boolean; onSelect: () => void }) {
  return (
    <button
      onClick={onSelect}
      className={`w-full text-left rounded-2xl border transition-all duration-200 overflow-hidden ${
        selected ? 'border-[#C97552]/50 bg-[#C97552]/5' : 'border-[#E8E0D6] bg-white hover:border-[#C97552]/30 hover:bg-[#F5F0EA]'
      }`}
    >
      <div className="px-4 py-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[#1A1A1A] text-sm font-semibold leading-snug">{place.name}</p>
            <p className="text-[#5C564E] text-xs mt-0.5">{place.area}</p>
            {selected && <p className="text-[#6b5f54] text-xs italic mt-1">"{place.tagline}"</p>}
          </div>
          <span className="text-[11px] text-[#6b5f54] flex-shrink-0 mt-1">{place.price_range}</span>
        </div>
        {selected && (
          <div className="mt-3 pt-3 border-t border-[#E8E0D6] space-y-3">
            <p className="text-[#4A4440] text-sm leading-relaxed">{place.story}</p>
            <div className="flex flex-wrap gap-2">
              {place.best_time && <span className="text-[11px] font-label px-2.5 py-1 rounded-full border border-[#E8E0D6] bg-white text-[#5C564E]">🕐 {place.best_time}</span>}
            </div>
            {place.insider_tip && (
  ...
  return (
    <button onClick={toggle} disabled={disabled} title={listening ? 'Stop' : 'Speak'}
      className={`flex-shrink-0 w-12 h-12 rounded-full border flex items-center justify-center transition-all ${
        listening ? 'bg-[#C97552] border-[#C97552] shadow-lg shadow-[#C97552]/25 animate-pulse' : 'bg-white border-[#D8D0C4] hover:border-[#C97552]/50 hover:bg-[#F0EBE3]'
      } disabled:opacity-40 disabled:cursor-not-allowed`}>
      <span className={`text-lg ${listening ? 'text-[#1A1A1A]' : 'text-[#4A4440]'}`}>🎤</span>
    </button>
  )
}
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] flex flex-col items-center justify-center px-6 pb-24">
      <div className="text-center max-w-xs">
        ...
        <p className="text-[#5C564E] text-sm">{steps[step]}</p>
        <div className="flex justify-center gap-1.5 mt-5">
          {steps.map((_, i) => <div key={i} className={`w-1.5 h-1.5 rounded-full transition-all ${i <= step ? 'bg-[#C97552]' : 'bg-[#E2D8CC]'}`} />)}
        </div>
      </div>
...
function Disclaimer({ label }: { label: string }) {
  return (
    <div className="rounded-2xl border border-[#E8E0D6] bg-white px-4 py-3 mb-6">
      <p className="text-[#6b5f54] text-xs leading-relaxed">
        <span className="font-medium text-[#3A3430]">Before you go —</span> {label}
      </p>
    </div>
  )
}
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] pb-28">
      <div className="max-w-lg mx-auto px-4 pt-8">
        <button onClick={onReset} className="text-[#6b5f54] text-xs hover:text-[#3A3430] transition-colors mb-5">← New search</button>
        ...
        <h1 className="font-serif italic text-2xl text-[#1A1A1A] leading-tight mb-5">
          {result.spots.length} places to shoot.<br />
          <span className="text-[#5C564E]">Golden hour to the minute.</span>
        </h1>
        ...
        {hasMap && (
          <>
            <div className="rounded-2xl overflow-hidden border border-[#E8E0D6] mb-2" style={{ height: '280px' }}>
              <LeafletMap
                places={result.spots}
            ...
            <p className="text-[#9A8E7E] text-[11px] text-center mb-5 font-label tracking-wider">TAP A PIN TO SELECT · SCROLL FOR DETAILS</p>
          </>
        )}
        ...
        <button onClick={onReset} className="w-full py-4 rounded-full border border-[#D8D0C4] bg-white text-[#5A504A] text-sm hover:border-[#C97552]/50 hover:text-[#C97552] transition-all">
...
function RoadTripResult({ result, onReset }: { result: Extract<TripAskResponse, { mode: 'road_trip' }>; onReset: () => void }) {
  return (
    <div className="min-h-screen bg-[#FAF8F5] pb-28">
      <div className="max-w-lg mx-auto px-4 pt-8">
        <button onClick={onReset} className="text-[#6b5f54] text-xs hover:text-[#3A3430] transition-colors mb-5">← New trip</button>
        ...
        <p className="text-[#5C564E] text-sm mb-6">{result.route_summary}</p>
        ...
        <button onClick={onReset} className="w-full py-4 rounded-full border border-[#D8D0C4] bg-white text-[#5A504A] text-sm hover:border-[#C97552]/50 hover:text-[#C97552] transition-all">Plan another trip →</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] pb-28">
      <div className="max-w-lg mx-auto px-4 pt-8">
        ...
        {hasMap && (
          <>
            <div className="rounded-2xl overflow-hidden border border-[#E8E0D6] mb-2" style={{ height: '300px' }}>
              <LeafletMap places={result.places} center={result.map_center} selectedIndex={selectedIndex} onSelect={i => setSelectedIndex(i)} />
            </div>
            <p className="text-[#9A8E7E] text-[11px] text-center mb-5 font-label tracking-wider">TAP A PIN OR CARD TO EXPLORE</p>
          </>
        )}
        <div className="space-y-2 mb-8">
        ...
        <Disclaimer label="verify opening hours and availability directly with each place." />
        <button onClick={onReset} className="w-full py-4 rounded-full border border-[#D8D0C4] bg-white text-[#5A504A] text-sm hover:border-[#C97552]/50 hover:text-[#C97552] transition-all">Search somewhere else →</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] flex flex-col pb-24">
      ...
      <div className="max-w-lg mx-auto w-full px-4 flex-1 flex flex-col">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3.5 py-2 rounded-full border text-xs font-label transition-all ${
                activeMode === mode.key
                  ...
                  : 'bg-white border-[#E2D8CE] text-[#5A504A] hover:border-[#C97552]/40 hover:text-[#2A2420]'
              }`}
            >
              <span>{mode.icon}</span>
        ...
        <div className="mb-5">
          <div className="flex gap-3 items-end">
            <textarea
              ...
              rows={3}
              className="flex-1 bg-white border border-[#E2D8CE] rounded-2xl px-4 py-3.5 text-[#1A1A1A] placeholder:text-[#8A7E6E] text-sm resize-none focus:outline-none focus:border-[#C97552]/50 focus:bg-[#F5F0EA] transition-all leading-relaxed"
            />
        ...
        {/* Examples */}
        <div>
          <p className="text-[10px] text-[#9A8E7E] font-label tracking-widest uppercase mb-3">Try asking</p>
          <div className="space-y-2">
            {EXAMPLE_QUERIES.map((ex, i) => (
              <button key={i} onClick={() => setQuery(ex)}
                className="w-full text-left px-4 py-3.5 rounded-xl border border-[#E8E0D6] bg-white hover:border-[#C97552]/35 hover:bg-[#F5F0EA] transition-all text-sm text-[#5C564E] leading-relaxed">
                "{ex}"
              </button>
            ))}
          </div>
